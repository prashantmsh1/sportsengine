...
import { type Match, type Commentary } from "@repo/database";
...
interface SeedCommentary {
    ...
    runs?: number; // For cricket logic extraction
}
...
function isLiveMatch(match: Match | SeedMatch): boolean {
    const start =
        match.startTime instanceof Date ? match.startTime : parseDate(match.startTime as string);
    const end = match.endTime instanceof Date ? match.endTime : parseDate(match.endTime as string);
...
async function insertCommentary(matchId: number, entry: SeedCommentary): Promise<Commentary> {
    const payload: any = {
    ...
    const response = await fetch(`${API_URL}/matches/${matchId}/commentry`, {
        method: "POST",
        headers: { "content-type": "application/json" },
...
function normalizeCricketFeed(entries: SeedCommentary[], match: Match): SeedCommentary[] {
    const sorted = [...entries].sort((a, b) => {
        const inningsDiff = inningsRank(a.period) - inningsRank(b.period);
        if (inningsDiff !== 0) {
            return inningsDiff;
        }
        const seqA = Number.isFinite(a.sequence) ? (a.sequence as number) : Number.MAX_SAFE_INTEGER;
        const seqB = Number.isFinite(b.sequence) ? (b.sequence as number) : Number.MAX_SAFE_INTEGER;
        if (seqA !== seqB) {
            return seqA - seqB;
        }
...
function cloneCommentaryEntries(
    ...
    return entries.map((entry) => {
        ...
        if (entry.message) {
            next.message = replaceTrailingTeam(entry.message, replacements) ?? entry.message;
        }
        return next;
    });
}

function expandFeedForMatches(feed: SeedCommentary[], seedMatches: SeedMatch[]): SeedCommentary[] {
    ...
    for (const match of seedMatches) {
        ...
        expanded.push(...cloneCommentaryEntries(templateEntries, templateMatch, match));
    }
...
function buildRandomizedFeed(
    ...
    const buckets = new Map<number | null, SeedCommentary[]>();
    for (const entry of feed) {
    ...
    while (randomized.length < feed.length) {
        ...
        const bucket = buckets.get(choice);
